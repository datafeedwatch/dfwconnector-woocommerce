  ...
  protected function _query($sql, $fetchType, $fetchFields = false)
  {
    ...
    while (mysqli_errno($databaseHandle) == 2013) {
      ...
      if (isset($_REQUEST['set_names'])) {
        mysqli_set_charset($databaseHandle, sanitize_text_field($_REQUEST['set_names']));
      }
...
class M1_Config_Adapter implements M1_Platform_Actions
{
  ...
  {
    return DFWBC_STORE_BASE_DIR . DFWBC_BRIDGE_DIRECTORY_NAME . DIRECTORY_SEPARATOR
...
class M1_Config_Adapter_Wordpress extends M1_Config_Adapter
{
  ...
  protected function _setWpecommerceData()
  {
    ...
    if ($version != '') {
      ...
      $filePath = DFWBC_STORE_BASE_DIR . "wp-content" . DIRECTORY_SEPARATOR . "plugins" . DIRECTORY_SEPARATOR
        ...
        switch ($event['entity_type']) {
          case 'shipment':
            ...
            foreach ($event['tracking_info'] as $trackingInfo) {
              ...
              $note = sprintf( __( 'Tracking info was deleted for tracking provider %1$s with tracking number %2$s', 'woo-advanced-shipment-tracking' ), $trackingProvider, $trackingNumber);
              // Add the note
...
class M1_Bridge_Action_Query
{
  ...
  {
    $sqlSettings = $bridge->request->get_param('sql_settings');
  ...
  public function perform(M1_Bridge $bridge)
  {
    if (isset($_POST['query']) && isset($_POST['fetchMode'])) {
      $query = base64_decode(swapLetters(sanitize_text_field($_POST['query'])));
...
class M1_Bridge_Action_Multiquery
{
  ...
  public function perform(M1_Bridge $bridge)
  {
    if (isset($_POST['queries']) && isset($_POST['fetchMode'])) {
      ...
      $queries = json_decode(base64_decode(swapLetters(sanitize_text_field($_POST['queries']))));
      $count = 0;
...
class M1_Bridge_Action_CreateRefund
{
  ...
  private function _checkRequestKey($requestKey)
  {
    $request = wp_remote_post(
      DFWBC_BRIDGE_CHECK_REQUEST_KEY_LINK,
      [
        'method'      => 'POST',
        'timeout'     => 60,
        'redirection' => 5,
  ...
  public function perform(M1_Bridge $bridge)
  {
    ...
    $itemsData = json_decode(sanitize_text_field($_POST['items']), true);
...
class M1_Bridge_Action_Basedirfs
{
  ...
  public function perform(M1_Bridge $bridge)
  {
    echo DFWBC_STORE_BASE_DIR;
  }
}

define('DFWBC_BRIDGE_VERSION', '132');
